...
function LastFM(options){
	...
	var internalCall = function(params, callbacks, requestMethod) {
		...
		if (requestMethod == 'GET' && cache && cache.contains(hash) && ! cache.isExpired(hash)) {
        ...
        var params_string = params_list.join('&').replace(/%20/g, '+');

        var requestUrl, requestData;

        if (requestMethod == 'GET') {
            requestUrl = apiUrl + '?' + params_string;
            requestData = null;
        }
        else {
            requestUrl = apiUrl;
            requestData = params_string;
        }
            ...
            if (xhr.readyState == 4) {
				...
				if (response.error) {
					if (callbacks.error)
					    ...
					    throw new Error("Lastm.fm API request error: " + response.message + " (Error code: " + response.error + ")");
				}
				else {
				    if (requestMethod == 'GET' && cache) {
				        var expiration = cache.getExpirationTime(params);

				        if (expiration > 0)
        				    cache.store(hash, response, expiration);
                    }
