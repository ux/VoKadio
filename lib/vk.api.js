...
function VkSession(app_id, settings, request_auth_cb)
{
    ...
    this.data = function (new_data, updated_at)
    {
        if (new_data && new_data != data)
            setData(new_data, updated_at);

        return data;
    };

    this.forget  = function () { setData(null, null); };
    ...
    function setData(new_data, updated_at)
    {
        data = new_data;
        self.updatedAt = (typeof updated_at == 'undefined') ? (new Date()) : updated_at;
        ...
        if (self.exists())
            timeout_id = setTimeout(
                function () { self.refresh(true); },
                self.updatedAt.getTime() + (self.data().expires_in - 30) * 1000 - Date.now());
