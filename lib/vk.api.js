...
function VkQuery(session)
{
    if ( ! session instanceof VkSession)
        throw new Error('Session is not instance of VkSession');
...
function VkSession(app_id, settings)
{
    ...
    function setData(new_data)
    {
        ...
        data = new_data;
        updated_at = Date();
        that.dispatchEvent({type: VkSession.EVENT_SESSION_UPDATED, data: new_data});
    }
}
