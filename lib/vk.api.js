...
VkAPI.Query = function (session, api_url, default_retry_interval)
{
    ...
    // Replace certain Unicode characters with escape sequences. JavaScript handles
    // many characters incorrectly, either silently deleting them, or treating them as line endings.
    // This part of code is taken from https://github.com/douglascrockford/JSON-js/blob/master/json2.js

    function fix_json_string(text)
    {
        json_cx.lastIndex = 0;

        if (json_cx.test(text)) {
            return text.replace(json_cx, function (a) {
    ...
    this.call = function (method, params, callbacks, start_retry_interval)
    {
        ...
        function call_method(retry_interval, allow_update_session)
        {
            ...
            function xhr_ready_state_changed()
            {
                ...
                var response_text = fix_json_string(this.responseText),
                    response;
                ...
                if (response.error) {
                    ...
                    switch (error.code) {
                        ...
                        case 9: // Flood control enabled for this action.
                            VkAPI.logger.inform("ВКонтакте запретил данное действие на некоторое время, посчитав его использование Вами как флуд.");
                            error_cb(error);
                            break;
