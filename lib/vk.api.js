...
function VkQuery(session)
{
    ...
    this.call = function (method_name, params, callbacks, retry_timeout, allow_session_autoupdate)
    {
        ...
        if (this.session.isEmpty()) {
            if (allow_session_autoupdate)
                return refresh_session_and_call(method_name, params, callbacks, false);
            else
        ...
        xhr.onreadystatechange = function() {
            ...
            if (response.error) {
                var error = new VkError(response.error);

                switch (error.code) {
                    case 6: // Too many requests per second.
                        setTimeout(function () { that.call(method_name, params, callbacks, 2 * retry_timeout); }, retry_timeout);
                        break;
                    case 9: // Flood control enabled for this action.
                        alert("ВКонтакте ограничил использование данного действия на некоторое время, посчитав его флудом.");
                        break;
                    case 5: // User authorization failed.
                        if (allow_session_autoupdate) {
                            refresh_session_and_call(method_name, params, callbacks, true);
                            break;
                        }
                    case 7: // Permission to perform this action is denied by user. Need request permissions by refreshing the session.
                        if (allow_session_autoupdate) {
                            refresh_session_and_call(method_name, params, callbacks, false);
                            break;
                        }
    ...
    var refresh_session_and_call = function (method_name, params, callbacks, forget_session_on_timeout)
    {
        var forget_timeout;

        if (forget_session_on_timeout)
            forget_timeout = setTimeout(function () { that.session.forget(); }, 30000);

        var session_updated_listener = function () {
            clearTimeout(forget_timeout);
            that.session.removeEventListener(VkSession.EVENT_SESSION_UPDATED, session_updated_listener);
            that.call(method_name, params, callbacks, undefined, false); // Error will be thrown, if session was not refreshed and was forgotten.
        };

        that.session.addEventListener(VkSession.EVENT_SESSION_UPDATED, session_updated_listener);
        that.session.refresh();
    };
}
...
function VkSession(app_id, settings, request_auth_cb)
{
    ...
    var set_data_and_updated_at = function (new_data, updated_at)
    {
        refresh_status = 0;
        clearTimeout(refresh_status_timeout);
...
{
    Error.apply(this, [vk_error.error_msg]);
...
VkError.prototype.name = 'VkError';
