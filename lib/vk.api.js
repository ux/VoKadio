...
function VkQuery(session)
{
    ...
    this.call = function (method, params, callbacks, retry_timeout, allow_session_autoupdate)
    {
        ...
        var request_url = VkQuery.API_URL + encodeURIComponent(method) + '?' + params_list.join('&');
    ...
    var refresh_session_and_call = function (method, params, callbacks, forget_session_on_timeout)
    {
        ...
        if (forget_session_on_timeout)
            forget_timeout = setTimeout(function () { self.session.forget(); }, 30000);

        var session_updated_listener = function () {
            clearTimeout(forget_timeout);
            self.session.removeEventListener(VkSession.EVENT_SESSION_UPDATED, session_updated_listener);
            self.call(method, params, callbacks, undefined, false); // Error will be thrown, if session was not refreshed and was forgotten.
        };

        self.session.addEventListener(VkSession.EVENT_SESSION_UPDATED, session_updated_listener);
...
function VkSession(app_id, settings, request_auth_cb)
{
    ...
    var set_data_and_updated_at = function (new_data, updated_at)
    {
        ...
        if (self.exists()) {
            console.debug("Session data received (" + updated_at + "):", new_data);
            ...
            self.dispatchEvent({type: VkSession.EVENT_SESSION_RECEIVED, data: new_data});
        }
        else {
            console.debug("Session data forgotten");

            self.dispatchEvent({type: VkSession.EVENT_SESSION_FORGOTTEN});
        }
...
VkSession.EVENT_SESSION_RECEIVED  = 'session-received';
VkSession.EVENT_SESSION_FORGOTTEN = 'session-forgotten';
