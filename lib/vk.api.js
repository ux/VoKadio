...
//TODO: Multi-apps sessions update
//TODO: updateSession callback for VkSession
//TODO: More scrupulous vkontakte errors checking and handling

function VkQuery(session, api_url)
{
    if ( ! VkQuery.static_initialized ) {
        ...
        VkQuery.static_initialized = true;
    }
...
function VkSession(app_id, test_mode, settings)
{
    ...
    if ( ! VkSession.static_initialized ) {
        VkSession.EVENT_SESSION_UPDATED = 'session-updated';
        
        VkSession._sessions = [];
        
        VkSession.updateSessions = function (new_session)
        {
            for ( var i in VkSession._sessions )
                VkSession._sessions[i].session(new_session);
        };
        
        VkSession.static_initialized = true;
    }
    
    VkSession._sessions.push(this);
    ...
    {
        console.info('updating session: ', new_session);
        session = new_session;
        last_updated = Date();
        this_obj.dispatchEvent({ type: VkSession.EVENT_SESSION_UPDATED, session: new_session });
    }
}
